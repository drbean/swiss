...
# Last Edit: 2007 Sep 04, 12:28:42 PM
# $Id: /swiss/branches/c6pairs/Games/Tournament/Swiss/Procedure/FIDE.pm 1328 2007-09-04T04:32:39.393229Z greg  $
...
"$refloat: table @pluspos NOK. Floated Up $whichround{$refloat} rounds ago\n";
	...
	elsif ( $pair[0]-> score == $pair[1]->score )
	{
	    ...
	    my @id = map { $_->id } @pair;
	    return ERROR,
		msg => "S1's $id[0]'s score lower than S2's $id[1]'s", %args;
	}
    }
...
sub c10 {
    my $self   = shift;
    my %args   = @_;
    return C11, %args if $args{C13loop};
    my $index  = $args{index};
    my $groups = delete $args{brackets};
    my $group  = $groups->[$index];
...
sub c13 {
    ...
    my $group   = $groups->[$index];
    my $members = $group->members;
    if ( @$members == 1 and not $args{C13loop} ) {
        my $byer = $members->[0];
	my $id = $byer->id;
    ...
    if ( $index == 0 and $args{C13loop} )
    {
	return ERROR,
	msg => "All score brackets joined into one, but no pairings", %args;
    }
    ...
    # if ( $penultpPrime == 0 and $args{C13loop} ) {
    if ( $penultpPrime == 0 ) {
...
"Bracket $last: @lastMemberIds => Bracket $penult: @{[map {$_->id} @{$groups->[$index-1]->members}]}\n";
    ...
    if ( $penultpPrime > 0 ) {
	my @ids = map { $_->id } @{$penultimate->members};
        print "Re-pairing Bracket $penult, @ids. p=$penultpPrime. ";
...
sub c14 {
    ...
    if ( $pprime == 0 and $index == $#$groups-1 and $args{C13loop} )
    {
	delete $args{index};
	return ( C13, index => $#$groups, brackets => $groups, %args );
    }
...
sub previousBracket {
    ...
    elsif ( $self->{previousBracket} ) { pop @{$self->{brackets}}; }
}
