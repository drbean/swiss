...
# Last Edit: 2007 Oct 05, 09:11:50 AM
# $Id: /swiss/branches/lastgp/lib/Games/Tournament/Swiss/Procedure/FIDE.pm 1441 2007-10-05T01:13:37.139811Z greg  $
...
=cut 
	...
	if (@unpairables) {
	    ...
	    print qq/Bracket $bracket. Floating @floaters/;
	    my @originals = map {$_->id} @{$groups->[$index]->members};
	    my @new = map {$_->id} @{$groups->[$index+1]->members};
            print "[$bracket] @originals => [$next] @new\n";
...
sub c4 {
    ...
    my $s1      = $group->s1;
    my $s2      = $group->s2;
    if ( defined $args{bigLastGroup} and $args{bigLastGroup} == $index )
    {
	for my $member ( @$members )
	{
	    my $score = $member->score;
	    my $reverseScore = -$score;
	    $member->score($reverseScore);
	}
	$group->members($members);
	my $p = $group->p;
	$s1 = $group->s1([@$members[$p..$#$members]]);
	$s2 = $group->s2([@$members[0..$p-1]]);
    }
    print
...
"Bracket @{[$index+1]}'s Remainder Group: @{[map{$_->id}@$nonpaired]}\n";
    ...
    return (C7, brackets => $groups, %args)
	if $args{bigLastGroup} and $args{bigLastGroup} == $index;
...
sub c12 {
	...
	return C2, brackets => $groups, %args;
    }
    my $members = $group->members;
...
"Bracket $last: @lastMemberIds => Bracket $penult: @{[map {$_->id} @{$groups->[$index-1]->members}]}\n";
	...
	delete $args{bigLastGroup};
	my $newLastIndex = $index - 1;
        return C1,
	  bigLastGroup => $newLastIndex,
          index => $newLastIndex,
          matches  => $matches,
          brackets => $groups,
          %args;
...
sub colors {
    ...
    my $diff = $self->meanScoreDifference;
    my $var = $self->differenceVariance;
    print "B3 mean=$diff var=$var\n";
...
=head2 _unroundedMeanScoreDifference

	$pairing->_unroundedMeanScoreDifference

The sum of the differences of the score of each player with that of its opponent, divided by the number of players so far paired, returned with an accuracy of as many decimal places as the machine will allow.

=cut

sub _unroundedMeanScoreDifference {
    my $self = shift;
    my @bracketed = @{$self->matches};
    my ($tablesPaired, $totalDiff) = (0) x 2;
    for my $bracket ( @bracketed )
    {   
	my @matches = @$bracket;
	for my $match ( @matches)
	{
	    my $contestants = $match->contestants;
	    next if grep /Bye/, keys %$contestants;
	    my @players = $match->myPlayers;
	    my $difference=0;
	    for my $number ( 0..$#players )
	    {
		my $score = $players[$number]->score;
		my $odd = $number % 2;
		$difference += $score if $odd;
		$difference -= $score unless $odd;
	    }
    ...
    my $meanScoreDifference = $totalDiff / $tablesPaired;
    return $meanScoreDifference;
}


=head2 meanScoreDifference

	$pairing->meanScoreDifference

The sum of the differences of the score of each player with that of its opponent, divided by the number of players so far paired, or equivalently, the sum of the differences of the scores of the players paired at each table, divided by the number of tables so far paired.

=cut

sub meanScoreDifference {
    my $self = shift;
    my $meanScoreDifference = $self->_unroundedMeanScoreDifference;
    return sprintf("%.2f", $meanScoreDifference);
}


=head2 differenceVariance

	$pairing->differenceVariance

The variation in the differences of each player's score with that of their opponent, computed as the average squared deviation of each difference from the mean. The lower the variance, the better.

=cut

sub differenceVariance {
    my $self = shift;
    my @bracketed = @{$self->matches};
    my ($tablesPaired, $totalDeviation) = (0) x 2;
    my $mean = $self->_unroundedMeanScoreDifference;
    for my $bracket ( @bracketed )
    {   
	my @matches = @$bracket;
	for my $match ( @matches)
	{
	    ...
	    my $squaredDeviation = $difference ** 2;
	    $totalDeviation += $squaredDeviation;
	    $tablesPaired++;
	}
    }
    my $variance = $totalDeviation / $tablesPaired;
    return sprintf("%.2f", $variance);
}
